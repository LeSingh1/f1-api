...
const WEB  = "https://site.web.api.espn.com/apis/common/v3/sports/racing/f1";
...
async function get(url) {
  const res = await fetch(url, { headers: { "user-agent": "f1-api (github.com/LeSingh1/f1-api)" } });
  if (!res.ok) throw new Error(`ESPN ${res.status} for ${url}`);
  return res.json();
}

// Formula 1 is racing/f1 on ESPN.
