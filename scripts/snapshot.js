...
if (games.length === 0) {
  console.log(`${ymd}: no Formula 1 games today; leaving snapshot unchanged`);
  process.exit(0);
}
mkdirSync("data", { recursive: true });
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "racing", league: "f1", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} Formula 1 games -> data/latest.json`);
