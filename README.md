...
Small, unofficial Node client for ESPN's Formula 1 data. ESPN serves Formula 1
under `racing/f1`, and this wraps those endpoints.

Part of the [espn-api](https://github.com/LeSingh1/espn-api) family. The parent
repo is one client for every sport. This one is just Formula 1, in the style of
...
```
git clone https://github.com/LeSingh1/f1-api
cd f1-api
node examples/scoreboard.js   # Formula 1 events on the board
...
Formula 1 is an individual sport, so `teams`, `roster` and `standings` mostly do not apply. Use `scoreboard`, `athlete`, `gamelog`, `splits`, `news`, and `summary`.
