...
is one client for every sport. This one is just Formula 1, in the style of
swar/nba_api.
...
cd f1-api
node examples/scoreboard.js   # Formula 1 events
npm test
```
...
Formula 1 is an individual sport, so `teams`, `roster` and `standings` mostly do not apply. Use `scoreboard`, `athlete`, `gamelog`, `splits`, `news`, `summary`, `odds`, and the parsed `scoreboardClean` / `gamelogClean`.

## Daily data

A scheduled GitHub Action runs once a day, pulls that day's Formula 1 games from
